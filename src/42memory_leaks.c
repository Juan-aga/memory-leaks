...
/*   Updated: 2023/02/26 20:02:44 by juan-aga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_memory_leaks.h"

static void	*ft_malloc_hook(t_mem_check *mem_check, int *fill_check_memory, t_mem_data data, int control);
...
*/	if (fill_check_memory)
	{
		...
		fill_check_memory = 0;
		if (control <= 1)
			return (ft_malloc_hook(&mem_check, &fill_check_memory, data, control));
		if (control == 2)
	...
	else
	{
		...
		if (control == 2)
			ft_real_free(to_free);//dlsym(RTLD_NEXT, "free"));//(__libc_free(to_free));
	}
	return (NULL);
}
